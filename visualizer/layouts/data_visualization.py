    ...
    return html.Div([
        html.Div([
            ...
            dbc.Container([
                html.Hr(),
                html.H3('Filter Tool'),                
                dbc.Row([dbc.Col(html.Label("Select Column for Filtering:"), width=5),
                            dbc.Col(html.Label("Enter Filter Condition:"), width=5),
                            dbc.Col(dbc.Button("+", id="add-condition-field-btn", n_clicks=0, color="primary"), width=1)]),
                html.Hr(),
                dbc.Row([
                dbc.Col(dcc.Dropdown(id={'type': 'column-selector', 'index': 0}, options=[{'label': col, 'value': col} for col in df.columns], style={'width': '100%'}), width=5),
                dbc.Col(dcc.Input(id={'type': 'filter-condition', 'index': 0}, type='text', placeholder='Enter text or a numeric range (e.g., "ISAR"/"<200"/"10,20")', style={'width': '100%'}), width=6),
                                ],style={'margin-bottom': '10px'}), #, id=f'condition-row-{count}')
                        html.Div(id='conditions-container')
                                ]),
                dbc.Button("List Filter Conditions", id="add-condition-btn", n_clicks=0, style={'marginRight': '10px'}),
                html.Div(id='conditions-display', className="mb-3"),
                html.Div("Error processing filter condition. Please check your input format.",id='filter-error-message', style={'color': 'red', 'display': 'none'}),              
                dbc.Row([dbc.Col(html.Div(id='buttons',children=[dbc.Button("Clear Filters", id="clear-filters-btn", n_clicks=0, style={'backgroundColor': '#FF4136', 'color': 'white'}),
                                dbc.Button("Filter Table", id="filter-table-btn", n_clicks=0, style={'backgroundColor': '#28a745', 'color': 'white', 'marginLeft': '10px'})])),
                            ]),
            html.Hr(),
            html.H3('Data Graphs'),                
            html.Label("Select Number of graphs you want:"),
            dbc.Row(dcc.Dropdown(id="num-graphs-dropdown",
                            options=[{'label': i, 'value': i} for i in range(1, 10)], 
                            style={'width': '100%','marginBottom': '5px'}), 
                            ),
            dbc.Row([
                    dbc.Col(dbc.Button("Export Graphs", id="export-button"),
                            width=3, style={'marginRight': '5px'}),
                    dbc.Col(dcc.Upload(id='import-upload',children=dbc.Button('Import Graph', id= 'import-btn'), multiple=False, style={'width': '100%'}),
                            width=3, style={'marginRight': '5px'}),
                    dbc.Col(dbc.Button("Show Graphs", id= 'show-graph', n_clicks=0),
                            width=3)
            ])], className="sidebar"),
        html.Div([
            ...
            '''),
            dcc.Store(id='accumulated-conditions-store'),  # Stores the accumulated conditions 
        ...
        html.Hr(),
            ...
            'display': 'flex',
            'flexWrap': 'wrap',
            'justifyContent': 'flex-start'},children=[]),
