...
# Callback to add a new condition fields:
    ...
    def add_condition_field(n_clicks,filtered_data ,existing_conditions, count):
        ...
        if n_clicks > 0:
            ...
            new_condition = dbc.Row([
                dbc.Col(dcc.Dropdown(id={'type': 'column-selector', 'index': count}, options=[{'label': col, 'value': col} for col in filtered_data.columns], style={'width': '60%'}), width=4),
                dbc.Col(dcc.Input(id={'type': 'filter-condition', 'index': count}, type='text', placeholder='Enter text or a numeric range (e.g., "ISAR"/"<200"/"10,20")', style={'width': '80%'}), width=5),
    ...
    def update_or_clear_conditions(add_clicks, clear_clicks, existing_conditions, selected_columns, filter_conditions):
        '''
    ...
    @app.callback(
        ...
        Input('accumulated-conditions-store', 'data')
        )
    def display_accumulated_conditions(conditions):
        ...
        conditions_list = [f"Condition {list(conditions.keys()).index(column)+1} is: '{column}': '{cond}'" for column, cond in conditions.items()]
    ...
    @app.callback(
        ...
        [Input('filter-table-btn', 'n_clicks'),
        Input('clear-filters-btn','n_clicks')],
        State('accumulated-conditions-store', 'data'),
    ...
    def filter_data(filter_btn, n_clicks2, existing_conditions, filtered_data):
        ...
        if filter_btn is None:
            ...
            if triggered_id == 'clear-filters-btn':
                ...
                return filtered_data.to_dict('records'), filtered_data.to_dict('records'), error_message
        ...
        # Handle numeric range. Expecting filter_condition to be "start,end"
            if ',' in cond:
                try:
                    ...
                    raise ValueError(f"Condition '{cond}' is not a valid numeric range for column '{col}'")
                ...
                if match:
                    ...
                    try:
                        ...
                        raise ValueError(f"Condition '{cond}' is not a valid numeric condition for column '{col}'")
                else:
                    try:
