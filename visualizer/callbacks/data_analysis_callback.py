from dash import html, dcc, Input, Output, callback_context , callback
...
def strip_values(value):
    ...
    elif value.replace('.', '', 1).isdigit() and value.count('.') < 2: # for float values in the input
        value = float(value)  
...
def register_callbacks(app, visualizer_instance):
        ...
        if n_clicks == 0:
            ...
            for trans in existing_inputs:
                ...
                if trans_name == 'ToTensor':
                    ...
                    continue
                trans_params = [trans['props']['children'][param]['props']['children'][0]['props']['children'][:-1]
                                for param in range(1,len(trans['props']['children']))]
                trans_values = [trans['props']['children'][value]['props']['children'][1]['props']['value']
            ...
            div = html.Div(html.H5('Transforms Applied!'),style={'color': 'green'}) if existing_inputs else html.Div(html.H5('No Transforms Selected!'),style={'color': 'red'})
    ...
    def display_transforms_values(selected_transforms, existing_inputs):
        ...
        for index, trans_name in enumerate(selected_transforms):
            ...
            if trans_id in inputs_dict:
                ...
                transform_div = html.Div(id=trans_id, children=[
                html.H5(f'{index+1}. {trans_name}'),
                *[html.Div([html.Label(f"{param_name}:"),
                            dcc.Input(id=f"{trans_name}.{param_name}",value=str(value),type='text')])
                            for param_name, value in default_values.items() if default_values]], style = {'marginBottom': '10px'})
    ...
    def update_layout(visualization_type):
        if visualization_type == 'img':
            ...
            return html.Div(id='image-div',children=[
                dbc.Row([  # This row contains both graphs side by side
                        dbc.Col(dcc.Graph(id='original-image-graph', style={"height": "50vh"}), width=6),
                        dbc.Col(dcc.Graph(id='transformed-image-graph', style={"height": "50vh"}), width=6),
    ...
    def display_raw_sample(selected_row, n_clicks, trans_dict):
        if visualizer_instance.vis_type == 'img' and selected_row:
            ...
            for row_idx in selected_row:
                ...
                org_fig.update_layout(title=f"{index_id} Original Image: ", margin=dict(l=10, r=10, t=30, b=10))
                if n_clicks and trans_dict['order']:
                    ...
                    trans_fig.update_layout(title=f"{index_id} Transformed Image: ", margin=dict(l=10, r=10, t=30, b=10))      
                    pred_fig = dataset.__getmodel__(row_idx,visualizer_instance.model) #get_predictions(dataset, label_gt, row_idx)
                    return org_fig, trans_fig, pred_fig, {"height": "60vh", 'width':'60vh','display': 'block'}, {"height": "50vh",'display': 'block'}, {'display': 'block'}
                else:
                    pred_fig = org_fig
                    trans_fig = org_fig
                    return org_fig, trans_fig, pred_fig, {'display': 'none'}, {"height": "60vh", 'width':'60vh','display': 'none'}, {'display': 'block'}
        else:
            raise PreventUpdate
    ...
    def plot_signals(selected_row, n_clicks):
        if visualizer_instance.vis_type== 'grph' and selected_row:
            ...
            if n_clicks and selected_row:
                build_transform = TransformsWrapper(trans_dict)
                trns = dataset.update_transformation(build_transform)
                signal_model_output, rpr_model_output = dataset.__getmodel__(row_idx, visualizer_instance.model)
                children = html.Div([signal_model_output,rpr_model_output])
            return signal_fig, rpr_gt_fig, children, {"height": "60vh","width":'60vh'}, {'display': 'block'}
        else:
            raise PreventUpdate    
