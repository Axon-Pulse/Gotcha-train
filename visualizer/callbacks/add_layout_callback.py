...
# Callback to handle file uploads and display file names
    @app.callback(
        Output('layout-upload-name', 'children'),
    ...
    @app.callback(
        Output('callback-upload-name', 'children'),
        [Input('upload-callback', 'filename')]
    )
    ...
    def update_output(n_clicks, layout_filename, layout_content, callback_filename, callback_content):
        if n_clicks == 0:
            raise PreventUpdate
        ...
        if layout_filename and layout_content:
            ...
            status_messages.append(html.Div(f"Layout script {layout_filename} saved to 'visualizer/layout/' directory.",style={'color': 'green'}))
        
        if callback_filename and callback_content:
            save_file(callback_filename, callback_content, os.getcwd()+'/visualizer/callbacks')
            status_messages.append(html.Div(f"Callback script {callback_filename} saved to 'visualizer/callbacks/' directory.",style={'color': 'green'}))
