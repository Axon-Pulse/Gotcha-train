...
def register_callbacks(app, visualizer_instance):
                    ...
                    new_child = html.Div(
                        [
                            ...
                            dcc.Dropdown(
                                ...
                                options=[
                                    {"label": "Data Graph", "value": "data-graph"},
                            ...
                            html.Div(
                                id={"type": "import-layout", "index": i}, style={"display": "none"}
                            ),
                            dcc.Graph(
                                id={"type": "import-graph", "index": i},
                                style={
                                    "height": "60vh",
                                    "width": "60vh",
                                    "borderRight": "3px solid RoyalBlue",
                                    "display": "none",
                                },
                            ),
                            dcc.Graph(
                        ...
                        id="dynamic-div",
                        style={"width": "30%", "minWidth": "300px", "padding": "10px"},
                    )
    ...
    def download_graph_config(n_clicks, data_graph_store, histogram_store, bar_plot_store):
        ...
        if n_clicks:
            data_to_save = {
                "data graph": data_graph_store,
                "histogram": histogram_store,
                "bar plot": bar_plot_store,
            }
    ...
    def create_import_layout(imported_data, full_data_store, existing_graphs):
        ...
        if triggered == "import-upload":
            ...
            filtered_data = next(
                (item["data"] for item in full_data["data_graph"] if "data" in item), None
            )  # Get the data table from the imported data
            ...
            if imported_data is not None:
                for i in range(
                    num_graphs
                ):  # Create the layout for each graph based on the number of graphs in the imported data
                    new_child = import_layout(i, data_df)
                    graphs = html.Div(
                        [
                            dcc.Graph(
                                id={"type": "import-graph", "index": i},
                                style={"height": "60vh", "width": "60vh", "display": "none"},
                            ),
                            dcc.Graph(
                                id={"type": "dynamic-graph", "index": i},
                                style={"height": "60vh", "width": "60vh", "display": "none"},
    ...
    def update_data_graph(generate_btn, graph_store, ids, y_axis, x_axis, filtered_data):
        ...
        if isinstance(graph_store, dict):  # Handling for newly created graphs
            ...
            import_flag = next(
                (item["import_flag"] for item in graph_store if "import_flag" in item), None
            )
        ...
        if import_flag == 1:
            for i, dg in enumerate(graph_store):
                if (
                    i == index and dg
                ):  # In order to manage the graph's places its crucial to verify that
                    ...
                    try:
                        ...
                        max_x = (
                            filtered_data[x_axis].max()
                            if pd.api.types.is_numeric_dtype(filtered_data[x_axis])
                            else len(filtered_data[x_axis].unique()) - 0.5
                        )
                        graph.add_shape(
                            type="line",
                            x0=max_x,
                            ...
                            y1=(
                                ...
                                if graph.layout.yaxis.range
                                else filtered_data[y_axis].max()
                            ),
                            line=dict(color="RoyalBlue", width=3),
                        )
                        visualizer_instance.imports = (
                            False  # To enable the user from adding new graphs
                        )
                        visualizer_instance.flag = (
                            False  # To enable repeating the process of importing the data
                        )
                        return (
                            graph,
                            {"display": "none"},
    ...
    def update_histogram(generate_btn, histogram_store, ids, selected_column, filtered_data):
        ...
        if isinstance(histogram_store, dict):  # Handling for newly created graphs
            ...
            import_flag = next(
                (item["import_flag"] for item in histogram_store if "import_flag" in item), None
            )
        ...
        if import_flag == 1:
            for i, hist in enumerate(histogram_store):
                if (
                    i == index and hist
                ):  # In order to manage the graphs places its crucial to verify that the index
                    ...
                    try:
                        ...
                        max_x = (
                            ...
                            else len(filtered_data[selected_column].unique()) - 0.5
                        )
                        graph.add_shape(
                            type="line",
                            x0=max_x,
                            ...
                            y1=(
                                graph.layout.yaxis.range[1]
                                if graph.layout.yaxis.range
                                else filtered_data[selected_column].value_counts().max()
                            ),
                            line=dict(color="RoyalBlue", width=3),
                        )
                        visualizer_instance.imports = (
            ...
            try:
                filtered_data = pd.DataFrame(filtered_data)
                graph = px.histogram(
                    filtered_data, x=selected_column, title=f"Histogram of: {selected_column}"
                )
                graph.update_layout(xaxis_type="category")
                updated_store = {
                    "axes": selected_column,
                    "data": filtered_data.to_dict("records"),
                    "import_flag": 0,
                }
                return (
    ...
    def update_facet(operation, layout_id, data):
        ...
        for opr in operation:
            dcc_obj = html.Div(
                id={"type": f"{opr}-div", "index": layout_id["index"]},
                children=[
                    html.Label(f"Enter {opr} column:"),
                    dcc.Dropdown(
                        id={"type": f"{opr}", "index": layout_id["index"]},
    ...
    def update_bar_plot(
        ...
        if isinstance(bar_plot_store, dict):  # Handling for newly created graphs
            ...
            import_flag = next(
                (item["import_flag"] for item in bar_plot_store if "import_flag" in item), None
            )
        if import_flag == 1:
            for i, bp in enumerate(bar_plot_store):
                if (
                    i == index and bp
                ):  # In order to manage the graphs places its crucial to verify that the index of
                    ...
                    try:
                        ...
                        facet_col = bp["axes"][3]
                        color_by = bp["axes"][4]
                        data_df["group_combination"] = (
                            data_df[group_by].astype(str).agg(", ".join, axis=1)
                        )
                        group_columns = (
                            ["group_combination"] + group_by + [color_by]
                            if color_by
                            else ["group_combination"] + group_by
                        )
                        ...
                        graph = generate_barplot(
                            bar_table, count_by, facet_row, facet_col, group_by, color_by
                        )  # Function to create the bar plot with the selected inputs (if there are any)
                        bp["import_flag"] = 0
                        visualizer_instance.imports = (
                            False  # To enable the user from adding new graphs
                        )
            ...
            try:
                ...
                if (
                    facet_type
                ):  # Check if facet is selected and get the inputs to update the bar plot accordingly
                    inputs = [
                        facet_inputs[i]["props"]["children"][1]["props"]["value"]
                        for i in range(len(facet_type))
                    ]
                ...
                updated_store = {
                    "axes": [group_by, count_by, facet_row, facet_col, color_by],
    ...
    def create_histogram_layout(layout_id, df):
        ...
        return html.Div(
            [
                ...
                dcc.Dropdown(
                    id={"type": "histogram-selector", "index": layout_id["index"]},
                ...
                html.Button(
                    "Generate Graph",
                    id={"type": "hist-btn", "index": layout_id["index"]},
                    n_clicks=0,
    ...
    def create_data_graph_layout(layout_id, df):
        ...
        layout = html.Div(
            [
                    ...
                    [
                        html.Div(
                            [
                                ...
                                dcc.Dropdown(
                                    id={"type": "x_axis-selector", "index": layout_id["index"]},
                                    options=[{"label": col, "value": col} for col in df.columns],
                                    value="id",
                                    style={"width": "200px"},
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Dropdown(
                                    id={"type": "y_axis-selector", "index": layout_id["index"]},
                                    options=[{"label": col, "value": col} for col in df.columns],
                                    value=[],
                                    style={"width": "200px"},
                                ),  # Adjust width as needed
                ...
                html.Button(
                    "Generate Graph",
                    id={"type": "data_graph-btn", "index": layout_id["index"]},
                    n_clicks=0,
                    style={
                        "marginTop": "20px",
                        "display": "block",
            ...
            [
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            html.Div(
                                [
                                    dbc.Checklist(
                                        id={
                                            "type": "facet-checklist",
                                            "index": layout_id["index"],
                                        },
                                        options=[
                                            {"label": "Facet Row", "value": "facet_row"},
                        ...
                        dbc.Col(
                            html.Div(
                                id={"type": "facet-input", "index": layout_id["index"]},
                                style={"width": "100%"},
                            )
                        ),
                    ]
                ...
                html.Div(
                    [
                        dcc.Dropdown(
                            id={"type": "groupby-selector", "index": layout_id["index"]},
                            options=[{"label": col, "value": col} for col in df.columns],
                            multi=True,
                            value=[],
                            style={
                        ...
                        dcc.Dropdown(
                            id={"type": "count-by-selector", "index": layout_id["index"]},
                            options=[{"label": col, "value": col} for col in df.columns],
                            style={
                                "width": "100%",
                                "marginRight": "3px",
                        ...
                        dcc.Dropdown(
                            id={"type": "color-by-selector", "index": layout_id["index"]},
                            options=[{"label": col, "value": col} for col in df.columns],
                            style={
                                "width": "100%",
                                "marginRight": "3px",
                ...
                html.Button(
                    "Generate Graph",
                    id={"type": "barplot-btn", "index": layout_id["index"]},
                    n_clicks=0,
                    style={
                        "marginTop": "20px",
                        "display": "block",
