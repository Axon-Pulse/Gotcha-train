...
from src.utils.utils import get_label_map_from_labels_dict
...
class GenericDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        label_dict = {label_key: row[label_key] for label_key in self.dataset_keys.label_keys}
        label_dict["idx"] = idx
        raw_sample = self.get_sample_from_path(raw_sample_path)
        if self.transformation:
            raw_sample = self.transformation(raw_sample)
        label_dict["label"] = get_label_map_from_labels_dict(
            raw_sample.shape, {a: label_dict[a] for a in ["dg", "rg"]}
        )
